...
if (isset($_POST['phd-submit'])) {


    // escape form data and retrieve user ID and name
    $year = mysqli_real_escape_string($con, $_POST['year']);
    $program_applied = mysqli_real_escape_string($con, $_POST['program_applied']);
    $supervisor_name = mysqli_real_escape_string($con, $_POST['supervisor_name']);
    $supervisor_department = mysqli_real_escape_string($con, $_POST['supervisor_department']);
    $reg_year = mysqli_real_escape_string($con, $_POST['reg_year']);
   ...
   // create directories if they don't exist
    ...
    if (move_uploaded_file($_FILES['proof']['tmp_name'], $proof_path)) {
        ...
        $url = "http://localhost/Curricula/uploads/" . $department . "/" . $name . "/" . $program_type . "/" . basename($proof_path);
        // save URL to database
        $insert_data = "INSERT INTO phd(userid, name, department, year, program_applied, supervisor_name, supervisor_department, reg_year, reg_date, status, link, financial_support,proof, hod_approval, hod_remarks, admin_approval, admin_remarks) 
                        VALUES ('$userid', '$name', '$department', '$year', '$program_applied', '$supervisor_name', '$supervisor_department', '$reg_year', '$reg_date', '$status', '$link','$financial_support', '$url', 'Pending Approval', 'NIL', 'Pending Approval', 'NIL')";
        $result = mysqli_query($con, $insert_data);
        if ($result) {
            $seminar_id = mysqli_insert_id($con);
            $academic_year = date("Y") . "-" . (date("Y") + 1);
...
if (!empty($errors)) {
    $error_string = implode("<br>", $errors);
    $_SESSION['message'] = "Error: " . $error_string;
    header("Location: phd-add-new.php");
    exit(0);
}
